...
def transcribe_upload(file: UploadFile, *, lang: str = "en-US") -> SpeechToTextResult:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            subprocess.run(
                ["ffmpeg", "-y", "-i", str(input_path), "-ar", "16000", "-ac", "1", "-f", "wav", str(wav_path)],
                capture_output=True,
                check=True,
                timeout=30,
            )
        ...
        return SpeechToTextResult(transcript=recognizer.recognize_google(audio, language=lang))
        ...
        raise HTTPException(status_code=504, detail="Audio conversion timed out") from exc
