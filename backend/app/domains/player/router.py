...
@router.post(
    "/resolve-url",
    response_model=PlayerResolveUrlResponse,
    dependencies=[Depends(require_scope("write:player"))],
)
...
@router.post(
    "/queue",
    response_model=PlayerQueueResponse,
    dependencies=[Depends(require_scope("write:player"))],
)
...
@router.get(
    "/queue",
    response_model=PlayerQueueResponse,
    dependencies=[Depends(require_scope("read:player"))],
)
...
@router.post(
    "/queue/next",
    response_model=PlayerNextTrackResponse,
    dependencies=[Depends(require_scope("write:player"))],
)
...
@router.post(
    "/predownload/enqueue",
    response_model=PlayerPredownloadJobResponse,
    dependencies=[Depends(require_scope("write:player"))],
)
async def enqueue_predownload(
...
@router.get(
    "/predownload/status",
    response_model=PlayerPredownloadStatusResponse,
    dependencies=[Depends(require_scope("read:player"))],
)
...
@router.post(
    "/predownload/tick/{job_id}",
    response_model=PlayerPredownloadTickResponse,
    dependencies=[Depends(require_scope("write:player"))],
)
