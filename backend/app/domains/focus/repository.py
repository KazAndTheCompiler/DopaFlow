...
def close_orphaned_running(db_path: str) -> None:
    """Close any focus_sessions rows still marked running (no matching active session)."""

    with tx(db_path) as conn:
        conn.execute(
            """UPDATE focus_sessions SET ended_at = ?, status = 'stopped'
               WHERE status = 'running'
               AND id NOT IN (
                   SELECT session_id FROM focus_active_session WHERE session_id IS NOT NULL
               )""",
            (datetime.now(timezone.utc).isoformat(),),
        )
