...
async def _svc(settings: Settings = Depends(get_settings_dependency)) -> JournalService:
    return JournalService(
        ...
        backup_dir=settings.journal_backup_dir
...
@router.get(
    ...
    dependencies=[Depends(require_scope("read:journal"))],
)
...
@router.post(
    ...
    status_code=201,
    dependencies=[Depends(require_scope("write:journal"))],
)
async def save_entry(
    payload: JournalEntryCreate, svc: JournalService = Depends(_svc)
) -> JournalEntryRead:
...
async def patch_entry(
    entry_id: str, payload: JournalEntryPatch, svc: JournalService = Depends(_svc)
) -> JournalEntryRead:
    try:
...
async def get_entry(
    identifier: str, svc: JournalService = Depends(_svc)
) -> JournalEntryRead:
...
@router.delete(
    "/entries/{identifier}",
    response_model=JournalDeleteResponse,
    dependencies=[Depends(require_scope("write:journal"))],
)
...
async def lock_entry(
    entry_id: str, svc: JournalService = Depends(_svc)
) -> JournalEntryRead:
...
@router.get(
    "/entries/{identifier}/versions",
    response_model=list[JournalVersionSummary],
    dependencies=[Depends(require_scope("read:journal"))],
)
async def list_versions(
    identifier: str, svc: JournalService = Depends(_svc)
) -> list[JournalVersionSummary]:
    return svc.list_versions(identifier)


@router.get(
    "/entries/{identifier}/versions/{version_number}",
    response_model=JournalVersionDetail,
    dependencies=[Depends(require_scope("read:journal"))],
)
async def get_version(
    identifier: str, version_number: int, svc: JournalService = Depends(_svc)
) -> JournalVersionDetail:
...
async def analytics_summary(
    days: int = Query(default=90, ge=7, le=3650), svc: JournalService = Depends(_svc)
) -> dict:
...
async def analytics_heatmap(
    year: int = Query(default=0), svc: JournalService = Depends(_svc)
) -> dict:
...
@router.get(
    "/search",
    response_model=list[JournalSearchResult],
    dependencies=[Depends(require_scope("read:journal"))],
)
...
async def get_prompt(
    for_date: str, svc: JournalService = Depends(_svc)
) -> JournalPromptResponse:
...
async def export_range(
    ...
    if fmt == "zip":
        return Response(
            ...
            headers={
                "Content-Disposition": 'attachment; filename="journal-export.zip"'
            },
        )
    if fmt == "json":
        return result
...
async def export_zip(
    ...
    return Response(
        ...
        media_type="application/zip",
        headers={
            "Content-Disposition": f'attachment; filename="journal-{from_date}-to-{to_date}.zip"'
        },
    )
...
async def journal_to_card(
    payload: JournalToCardIn, db_path: str = Depends(_db_path)
) -> dict:
...
@router.post(
    "/transcribe",
    response_model=JournalTranscriptResponse,
    dependencies=[Depends(require_scope("write:journal"))],
)
async def transcribe_audio(
    file: UploadFile = File(...), lang: str = Query(default="en-US")
) -> JournalTranscriptResponse:
...
@router.get(
    "/graph",
    response_model=JournalGraphResponse,
    dependencies=[Depends(require_scope("read:journal"))],
)
...
@router.get(
    "/backup-status",
    response_model=JournalBackupStatus,
    dependencies=[Depends(require_scope("read:journal"))],
)
...
@router.post(
    "/backup/trigger",
    response_model=JournalBackupTriggerResponse,
    dependencies=[Depends(require_scope("write:journal"))],
)
async def trigger_backup(
    date: str | None = Query(default=None), svc: JournalService = Depends(_svc)
) -> JournalBackupTriggerResponse:
...
@router.post(
    "/export-today",
    response_model=JournalExportTodayResponse,
    dependencies=[Depends(require_scope("write:journal"))],
)
...
@router.get(
    "/templates",
    response_model=list[JournalTemplate],
    dependencies=[Depends(require_scope("read:journal"))],
)
...
@router.post(
    "/templates",
    response_model=JournalTemplate,
    status_code=201,
    dependencies=[Depends(require_scope("write:journal"))],
)
async def create_template(
    payload: JournalTemplateCreate, svc: JournalService = Depends(_svc)
) -> JournalTemplate:
...
async def get_template(
    template_id: str, svc: JournalService = Depends(_svc)
) -> JournalTemplate:
...
async def update_template(
    template_id: str, payload: JournalTemplatePatch, svc: JournalService = Depends(_svc)
) -> JournalTemplate:
...
@router.delete(
    "/templates/{template_id}",
    response_model=JournalTemplateDeleteResponse,
    dependencies=[Depends(require_scope("write:journal"))],
)
...
@router.post(
    ...
    response_model=JournalTemplateApplyResponse,
    dependencies=[Depends(require_scope("write:journal"))],
)
