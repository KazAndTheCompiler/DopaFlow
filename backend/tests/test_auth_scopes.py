...
async def test_remote_tasks_list_accepts_read_scope(tmp_path: Path) -> None:
    ...
    response = await _request(
        app, "GET", "/api/v2/tasks/", headers=_bearer("read:tasks")
    )
...
@pytest.mark.anyio
async def test_remote_calendar_and_journal_read_routes_accept_matching_scopes(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
    ...
    calendar_response = await _request(
        app, "GET", "/api/v2/calendar/events", headers=_bearer("read:calendar")
    )
    journal_response = await _request(
        app, "GET", "/api/v2/journal/templates", headers=_bearer("read:journal")
    )
...
@pytest.mark.anyio
async def test_remote_focus_and_projects_require_matching_scopes(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
...
async def test_remote_focus_and_projects_accept_matching_scopes(tmp_path: Path) -> None:
    ...
    focus_response = await _request(
        app, "GET", "/api/v2/focus/status", headers=_bearer("read:focus")
    )
    projects_response = await _request(
        app, "GET", "/api/v2/projects/", headers=_bearer("read:projects")
    )
...
@pytest.mark.anyio
async def test_remote_review_and_insights_require_matching_scopes(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
...
async def test_remote_board_view_uses_task_read_scope(tmp_path: Path) -> None:
    ...
    allowed = await _request(
        app, "GET", "/api/v2/boards/eisenhower", headers=_bearer("read:tasks")
    )
...
@pytest.mark.anyio
async def test_remote_gamification_routes_require_matching_scopes(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
    ...
    status_allowed = await _request(
        app, "GET", "/api/v2/gamification/status", headers=_bearer("read:gamification")
    )
...
async def test_remote_nutrition_routes_require_matching_scopes(tmp_path: Path) -> None:
    ...
    read_allowed = await _request(
        app, "GET", "/api/v2/nutrition/foods", headers=_bearer("read:nutrition")
    )
...
async def test_remote_route_rejects_tampered_bearer_token(tmp_path: Path) -> None:
    ...
    response = await _request(
        app, "GET", "/api/v2/tasks/", headers={"Authorization": f"Bearer {tampered}"}
    )
...
async def test_admin_ops_can_issue_scope_token_for_remote_use(tmp_path: Path) -> None:
    ...
    follow_up = await _request(
        app, "GET", "/api/v2/tasks/", headers={"Authorization": f"Bearer {token}"}
    )
...
async def test_admin_ops_can_list_and_revoke_scope_tokens(tmp_path: Path) -> None:
    ...
    listed = await _request(
        app, "GET", "/api/v2/ops/auth-tokens", headers=_ops_headers("admin:ops")
    )
    assert listed.status_code == 200
    assert any(
        item["id"] == token_id and item["subject"] == "qa-revoke"
    ...
    revoked = await _request(
        app,
        "DELETE",
        f"/api/v2/ops/auth-tokens/{token_id}",
        headers=_ops_headers("admin:ops"),
    )
...
@pytest.mark.anyio
async def test_remote_commands_and_search_accept_matching_tokens(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
    ...
    commands_response = await _request(
        app, "GET", "/api/v2/commands/list", headers=_bearer("read:commands")
    )
    search_response = await _request(
        app, "GET", "/api/v2/search?q=test", headers=_bearer("read:search")
    )
...
@pytest.mark.anyio
async def test_remote_alarm_and_player_routes_accept_matching_tokens(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
    ...
    alarm_response = await _request(
        app, "GET", "/api/v2/alarms", headers=_bearer("read:alarms")
    )
    player_response = await _request(
        app, "GET", "/api/v2/player/queue", headers=_bearer("read:player")
    )
...
async def test_remote_ops_backup_and_seed_require_admin_scope(tmp_path: Path) -> None:
    ...
    backup_denied = await _request(
        app, "GET", "/api/v2/ops/backup/db", headers=_ops_headers("write:ops")
    )
    seed_denied = await _request(
        app, "POST", "/api/v2/ops/seed", headers=_ops_headers("write:ops")
    )
    backup_allowed = await _request(
        app, "GET", "/api/v2/ops/backup/db", headers=_ops_headers("admin:ops")
    )
...
@pytest.mark.anyio
async def test_packaged_ops_routes_require_matching_ops_secret(
...
@pytest.mark.anyio
async def test_remote_docs_and_openapi_are_disabled_when_dev_auth_is_off(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
...
@pytest.mark.anyio
async def test_remote_request_cannot_bypass_scope_check_with_localhost_host_header(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
    ...
    response = await _request(
        app, "GET", "/api/v2/tasks/", headers={"Host": "localhost:8000"}
    )
...
@pytest.mark.anyio
async def test_scope_tokens_cannot_access_endpoints_outside_declared_scope(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
    ...
    read_journal = await _request(
        app, "GET", "/api/v2/journal/templates", headers=headers
    )
    delete_habit = await _request(
        app, "DELETE", "/api/v2/habits/hab_missing", headers=headers
    )
...
@pytest.mark.anyio
async def test_loopback_browser_origin_can_be_trusted_when_flag_enabled(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    monkeypatch.setenv("DOPAFLOW_DB_PATH", str(db_path))
    ...
    async with httpx.AsyncClient(
        ...
        response = await client.get(
            "/api/v2/tasks/", headers={"Origin": "http://localhost:5173"}
        )
...
@pytest.mark.anyio
async def test_remote_request_cannot_bypass_scope_check_with_spoofed_app_origin(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    run_migrations(str(db_path))
    ...
    response = await _request(
        app, "GET", "/api/v2/tasks/", headers={"Origin": "app://dopaflow"}
    )
...
@pytest.mark.anyio
async def test_loopback_client_still_requires_scope_without_trust_flag(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    db_path = tmp_path / "scopes.sqlite"
    monkeypatch.setenv("DOPAFLOW_DB_PATH", str(db_path))
...
@pytest.mark.anyio
async def test_legacy_scope_env_flags_still_work_as_fallback(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
