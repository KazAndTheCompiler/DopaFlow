...
async def test_ops_routes_return_403_not_401(tmp_path: Path) -> None:
    ...
    for path in ("/api/v2/ops/stats", "/api/v2/ops/config", "/api/v2/ops/sync-status"):
        ...
        wrong = await _request(
            app, "GET", path, headers={"X-Ops-Secret": "wrong-secret"}
        )
        ...
        assert missing.json().keys() == {"detail"}, (
            f"{path} missing secret body keys mismatch"
        )
        assert wrong.json().keys() == {"detail"}, (
            f"{path} wrong secret body keys mismatch"
        )
