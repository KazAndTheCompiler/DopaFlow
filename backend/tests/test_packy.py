...
class TestPackyVoiceCommandMode:
    ...
    def test_voice_command_executes_task_when_auto_execute_true(
        self, packy_service: PackyService, db_path: str
    ) -> None:
        ...
        # Verify response indicates execution happened
        assert res.mode == "executed", f"Expected mode=executed, got {res.mode}"
        assert res.intent == "task.create"
    ...
    def test_voice_command_without_db_path_returns_error(
        self, packy_service: PackyService
    ) -> None:
        """Voice command with auto_execute=true but no db_path must return error."""
        from app.domains.packy.schemas import PackyVoiceCommand

        res = packy_service.voice_command(
            PackyVoiceCommand(text="add task buy milk", auto_execute=True)
        )
