...
class TestValidateIdToken:
    ...
    def test_expired_token(self):
        ...
        id_token = pyjwt.encode(payload, private_key, algorithm="RS256", headers={"kid": "test-key-1"})
        jwks = {"keys": [{"kty": "RSA", "kid": "test-key-1", "n": _get_rsa_n(private_key), "e": _get_rsa_e(private_key), "alg": "RS256", "use": "sig"}]}
        with pytest.raises(ValueError, match="ID token validation failed"):
            validate_id_token(id_token, "https://keycloak.example.com/realms/test", "dopaflow-client", jwks)
    ...
    def test_nonce_check(self):
        ...
        claims = validate_id_token(id_token, "https://keycloak.example.com/realms/test", "dopaflow-client", jwks, nonce="expected-nonce")
        assert claims["sub"] == "user-123"
        # Wrong nonce fails
        with pytest.raises(ValueError, match="Nonce mismatch"):
            validate_id_token(id_token, "https://keycloak.example.com/realms/test", "dopaflow-client", jwks, nonce="wrong-nonce")
...
class TestCaching:
    ...
    async def test_discovery_cache(self):
        doc = {"issuer": "https://test.example.com", "authorization_endpoint": "https://test.example.com/auth", "token_endpoint": "https://test.example.com/token", "jwks_uri": "https://test.example.com/jwks"}
        call_count = 0
