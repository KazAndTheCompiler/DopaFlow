...
test.beforeEach(async ({ page }) => {
  ...
  await page.route(`${apiBase}/focus/sessions/control**`, (route) =>
    route.fulfill(
      json({
        id: 'fs_1',
        status: 'completed',
  ...
  await page.route(`${apiBase}/calendar/sync/status**`, (route) =>
    route.fulfill(json({ ok: true, conflicts: 0, status: 'healthy' })),
  );
  ...
  await page.route(`${apiBase}/notifications**`, (route) =>
    route.fulfill(
      json([
        {
          id: 'ntf_1',
  ...
  await page.route(`${apiBase}/packy/**`, (route) =>
    route.fulfill(
      json({ text: 'Keep the surface clean.', tone: 'helpful', suggested_action: null }),
    ),
  );
  ...
  await page.route(`${apiBase}/insights/weekly-digest**`, (route) =>
    route.fulfill(json({ title: 'Weekly snapshot', highlights: ['Momentum is holding steady.'] })),
  );
  ...
  await page.route(`${apiBase}/gamification/status**`, (route) =>
    route.fulfill(
      json({
        level: {
          total_xp: 40,
  ...
  await page.route(`${apiBase}/digest/**`, (route) =>
    route.fulfill(
      json({
        score: 78,
  ...
  await page.route(`${apiBase}/integrations/status**`, (route) =>
    route.fulfill(
      json({
  ...
  await page.route(`${apiBase}/vault/conflicts/*/preview`, (route) =>
    route.fulfill(
      json({
        record: {
          id: 1,
  ...
  await page.route(`${apiBase}/vault/tasks/import-preview**`, (route) =>
    route.fulfill(
      json({
        importable: [
          {
  ...
  await page.route(`${apiBase}/vault/tasks/import-confirm**`, (route) =>
    route.fulfill(
      json({
        imported: 1,
        updated: 0,
  ...
  await page.route(`${apiBase}/vault/push/daily-tasks/**`, (route) =>
    route.fulfill(
      json({
        pushed: 1,
        skipped: 0,
...
test('digest surface explains momentum instead of only showing raw counters', async ({ page }) => {
  ...
  await expect(page.getByText('You kept momentum moving in the right direction.')).toBeVisible({
    timeout: 15_000,
  });
  await expect(page.getByText('What this period says')).toBeVisible({ timeout: 15_000 });
  await expect(page.getByText('Execution is visible with 4 completed tasks.')).toBeVisible({
    timeout: 15_000,
  });
...
test('calendar and settings surfaces render updated UI', async ({ page }) => {
  ...
  await expect(page.locator('nav button:has-text("Calendar")').first()).toBeVisible({
    timeout: 15_000,
  });
  ...
  await expect(page.getByText('Calendar Sharing', { exact: true })).toBeVisible({
    timeout: 15_000,
  });
  await expect(page.getByText('Cross-install sharing')).toBeVisible({ timeout: 15_000 });
});

test('vault settings surface exposes daily section and import actions when bridge is enabled', async ({
  page,
}) => {
  await page.route(`${apiBase}/vault/status**`, (route) =>
    route.fulfill(
  ...
  await expect(page.getByText('Daily task section', { exact: true })).toBeVisible({
    timeout: 15_000,
  });
  await expect(page.getByText('Import tasks from vault', { exact: true })).toBeVisible({
    timeout: 15_000,
  });
  ...
  await expect(page.getByText('+ [ ] Vault edited', { exact: true })).toBeVisible({
    timeout: 15_000,
  });
  await expect(page.getByText('- [ ] App snapshot', { exact: true })).toBeVisible({
    timeout: 15_000,
  });
});
