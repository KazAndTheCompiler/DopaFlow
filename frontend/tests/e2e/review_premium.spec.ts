...
test.describe('Review premium — card interaction', () => {
  test('review surface renders due card', async ({ page }) => {
    ...
    await page.route(`${apiBase}/review/decks**`, (route) =>
      route.fulfill(json([{ id: 'deck_default', name: 'Default', card_count: 1 }])),
    );

    await page.goto('/#/review');
    await page.waitForTimeout(500);

    await expect(page.getByText('What is spaced repetition?').first()).toBeVisible({
      timeout: 8_000,
    });
  });
  ...
  test('Show Answer button reveals back of card', async ({ page }) => {
    ...
    await expect(
      page.getByText('A learning technique that uses increasing intervals between reviews.'),
  ...
  test('empty deck shows no-cards-due message', async ({ page }) => {
    ...
    await page.route(`${apiBase}/review/decks**`, (route) =>
      route.fulfill(json([{ id: 'deck_default', name: 'Default', card_count: 0 }])),
    );
  ...
  test('review surface loads without errors', async ({ page }) => {
    ...
    await setupRoutes(page);
    await page.route(`${apiBase}/review/cards**`, (route) =>
      route.fulfill(json([TODAY_DUE_CARD, REVIEWED_CARD])),
    );
    await page.route(`${apiBase}/review/decks**`, (route) =>
      route.fulfill(json([{ id: 'deck_default', name: 'Default', card_count: 2 }])),
    );
