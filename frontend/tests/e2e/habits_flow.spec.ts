...
test.describe("Habits flow regression", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route(`${apiBase}/habits**`, async (route) => {
      ...
      if (method === "GET" && (url.endsWith("/habits/") || url.endsWith("/habits") || (url.includes("/habits/") && !url.includes("/logs") && !url.includes("/checkin") && !url.includes("/freeze") && !url.includes("/unfreeze")))) {
      ...
      if (method === "POST" && url.includes("/checkin")) {
        ...
        habits = habits.map((h) =>
          h.id === id
            ? { ...h, current_streak: h.current_streak + 1, completion_pct: 100, last_checkin_date: new Date().toISOString(), updated_at: new Date().toISOString() }
            : h,
        );
  ...
  test("can create a habit and check it in", async ({ page }) => {
    ...
    await expect(
      page.getByRole("article").filter({ hasText: "Morning meditation" }).first().getByText("ST 1d", { exact: true }),
