...
test.describe("Route startup regression", () => {
  ...
  test("default route does not crash", async ({ page }) => {
    await page.goto("/");
  ...
  test("hash navigation to each surface does not crash", async ({ page }) => {
    ...
    for (const route of routes) {
      ...
      const hasError = await page.getByText("Surface failed to render").isVisible().catch(() => false);
      if (hasError) {
        throw new Error(`Route /${route} rendered an error boundary`);
      }
    }
  });
});
