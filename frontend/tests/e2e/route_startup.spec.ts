...
test.describe('Route startup regression', () => {
  ...
  test('default route lands on the today surface with runway guidance', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('main strong').filter({ hasText: 'Today' }).first()).toBeVisible({
      timeout: 15_000,
    });
  ...
  test('navigation between surfaces maintains state', async ({ page }) => {
    ...
    await expect(page.getByText('You have room to plan intentionally')).toBeVisible({
      timeout: 15_000,
    });
  });
});
...
test.describe('Command palette navigation', () => {
  ...
  test('ctrl/cmd+k opens the command palette and can navigate to focus', async ({
    page,
    browserName,
  }) => {
    await page.goto('/');
    ...
    await expect(page.getByText('Move fast without leaving the keyboard')).toBeVisible({
      timeout: 15_000,
    });
