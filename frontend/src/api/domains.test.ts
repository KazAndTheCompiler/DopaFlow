...
describe("integrations API", () => {
  it("connectGmail posts with payload", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ status: "connected", url: "http://callback" }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
...
describe("packy API", () => {
  ...
  it("getPackyWhisper returns whisper data", async () => {
    ...
    expect((result as unknown as Record<string, unknown>).transcript).toBe(
      "hello",
    );
  });
...
describe("projects API", () => {
  it("listProjects returns projects array", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify([{ id: "proj_1", name: "Work", color: "#fff" }]),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
  ...
  it("createProject posts with payload", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ id: "proj_2", name: "Personal", color: "#000" }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
  ...
  it("updateProject sends PATCH request", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ id: "proj_1", name: "Updated", color: "#abc" }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
...
describe("sharing API", () => {
  it("listShareTokens returns tokens", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify([{ id: "tok_1", label: "Work", token: "abc123" }]),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
  ...
  it("listPeerFeeds returns feeds", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify([
          { id: "feed_1", label: "Home", base_url: "http://localhost" },
        ]),
        {
          status: 200,
          headers: { "content-type": "application/json" },
...
describe("vault API", () => {
  it("getVaultStatus returns status", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ connected: true, last_sync: "2024-01-01" }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
...
describe("tasks API", () => {
  ...
  it("createTask posts with payload", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ ...mockTask, id: "task_2", title: "New task" }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
  ...
  it("quickAddTask posts with text", async () => {
    ...
    expect((result as unknown as Record<string, unknown>).preview).toBe(
      "Buy milk",
    );
    const [, options] = mockFetch.mock.calls[0];
    const body = JSON.parse(options.body as string);
  ...
  it("getTaskContext returns task with context", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ task: mockTask, dependencies: [], dependents: [] }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
...
describe("review API", () => {
  it("listReviewDecks returns decks", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify([{ id: "deck_1", name: "Main", card_count: 10 }]),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
  ...
  it("createReviewDeck posts with name", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ id: "deck_2", name: "Spanish", source_type: null }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
...
describe("journal API", () => {
  ...
  it("deleteJournalEntry sends DELETE", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ deleted: true, identifier: "2024-01-01" }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
  ...
  it("exportJournalToday posts to export endpoint", async () => {
    mockFetch.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ path: "/exports/2024-01-01.md", entry_count: 3 }),
        {
          status: 200,
          headers: { "content-type": "application/json" },
        },
