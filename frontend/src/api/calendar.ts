...
export async function createCalendarEvent(
  ...
  return parseApiSchema<CalendarEvent>(
    calendarEventSchema,
    await apiClient<unknown>("/calendar/events", {
      method: "POST",
...
export function syncGoogleCalendar(payload: {
  fetch_from?: string;
}): Promise<{ status: string }> {
  return apiClient("/calendar/google/sync", {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
...
export function getGoogleCalendarOAuthUrl(
  ...
  return apiClient(
    `/calendar/oauth/url?redirect_uri=${encodeURIComponent(redirect_uri)}`,
  );
}
...
export function getCalendarSyncStatus(): Promise<{
  ...
  return apiClient<{ ok: boolean; conflicts: number; status: string }>(
