...
export async function listTasks(sortBy?: string): Promise<Task[]> {
  ...
  return parseApiSchema<Task[]>(
    tasksSchema,
    await apiClient<unknown>(`/tasks/${queryString ? `?${queryString}` : ''}`),
  );
}
...
export async function createTask(payload: Partial<Task>): Promise<Task> {
  return parseApiSchema<Task>(
    taskSchema,
    await apiClient<unknown>('/tasks/', { method: 'POST', body: JSON.stringify(payload) }),
  );
}

export function quickAddTask(payload: TaskQuickAddInput): Promise<TaskQuickAddPreview> {
  return apiClient<TaskQuickAddPreview>('/tasks/quick-add', {
    method: 'POST',
...
export async function updateTask(id: string, patch: Partial<Task>): Promise<Task> {
  return parseApiSchema<Task>(
    taskSchema,
    await apiClient<unknown>(`/tasks/${id}`, { method: 'PATCH', body: JSON.stringify(patch) }),
  );
}
...
export function bulkCompleteTask(ids: string[]): Promise<{ updated: number }> {
  return apiClient<{ updated: number }>('/tasks/bulk/complete', {
    method: 'POST',
...
export function bulkDeleteTask(ids: string[]): Promise<{ updated: number }> {
  return apiClient<{ updated: number }>('/tasks/bulk/delete', {
    method: 'POST',
    body: JSON.stringify({ ids }),
  });
}
