...
export function exportJournalToday(): Promise<{
  path: string;
  entry_count: number;
}> {
  return apiClient<{ path: string; entry_count: number }>(
    "/journal/export-today",
    {
      method: "POST",
    },
  );
...
export function listJournalTemplates(): Promise<
...
export function applyJournalTemplate(
  ...
  return apiClient<{ body: string; tags: string[] }>(
    `/journal/templates/${templateId}/apply`,
    {
      method: "POST",
    },
  );
