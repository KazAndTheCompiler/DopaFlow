...
export async function updateVaultConfig(update: VaultConfigUpdate): Promise<VaultConfig> {
  return apiClient<VaultConfig>("/vault/config", {
    method: "PATCH",
...
export async function rollbackVaultFile(recordId: number): Promise<VaultRollbackResult> {
  return apiClient<VaultRollbackResult>(`/vault/rollback/${recordId}`, { method: "POST" });
}
...
export async function pushDailyTasksSection(date: string): Promise<VaultPushResult> {
  return apiClient<VaultPushResult>(`/vault/push/daily-tasks/${date}`, { method: "POST" });
}
...
export async function confirmTaskImport(candidates: TaskImportCandidate[]): Promise<ImportResult> {
