...
export function useCommandExecutor(deps: CommandExecutorDeps): UseCommandExecutorResult {
  const { tasks, journal, calendar, focus, alarms, habits, review, packy } = deps;
  ...
  const execute = useCallback(async (
    ...
    try {
      ...
      if (status === "greeting" || status === "help" || status === "unknown" || status === "ok") {
        try {
          const packyResult = await packy.ask(raw, { route: currentRoute ?? "today" });
