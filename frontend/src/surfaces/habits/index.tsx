...
export default function HabitsView(): JSX.Element {
  ...
  const checkedInHabits = habits.habits.filter(
    (habit) => (habit.completion_pct ?? 0) >= 100,
  ).length;
  const bestStreak = habits.habits.reduce(
  ...
  const handleAdd = async (): Promise<void> => {
    ...
    try {
      ...
      showToast(
        "Could not create the habit. Check the server is running.",
        "error",
      );
    } finally {
      setBusy(false);
    ...
    <div style={{ display: "grid", gap: "1rem" }}>
      <section
        style={{
          ...
          border:
            "1px solid color-mix(in srgb, var(--accent) 16%, var(--border-subtle))",
          display: "grid",
          gap: "0.9rem",
        }}
      >
      ...
      <section
        style={{
          ...
          background:
            "linear-gradient(160deg, color-mix(in srgb, var(--surface) 93%, white 7%), color-mix(in srgb, var(--surface) 98%, black 2%))",
          borderRadius: "20px",
          border: "1px solid var(--border-subtle)",
          display: "grid",
          gap: "0.9rem",
      ...
      >
        <div style={{ display: "grid", gap: "0.25rem", maxWidth: "64ch" }}>
          ...
          >
            Aim for something obvious enough to survive low-energy days. One
            durable habit beats five abandoned ones.
          </span>
        </div>
        <div
          style={{
      ...
      <HabitsPanel
        ...
        onCheckIn={(id) => void habits.checkIn(id)}
        onRefresh={() => void habits.refresh()}
      />
      {habits.habits.length > 0 && (
        <StreakHeatmap habits={habits.habits} checkins={checkins} />
      )}
