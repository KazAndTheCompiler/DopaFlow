...
export function HabitsPanel({ habits, loading = false, onCheckIn, onRefresh }: HabitsPanelProps): JSX.Element {
  if (loading) {
    return <SkeletonList rows={4} showAvatar />;
  }

  if (habits.length === 0) {
    return <EmptyState icon="HB" title="No habits yet" subtitle="Build consistency — add your first habit above." />;
  }
