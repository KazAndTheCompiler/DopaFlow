...
export default function OverviewView(): JSX.Element {
  ...
  const tasksDueToday = useMemo(
    () =>
      tasks.tasks.filter(
        (t) => !t.done && t.due_at && t.due_at.slice(0, 10) <= todayIso,
      ).length,
  ...
  const tasksCompletedToday = useMemo(
    () =>
      tasks.tasks.filter(
        (t) => t.done && t.updated_at?.slice(0, 10) === todayIso,
      ).length,
    [tasks.tasks, todayIso],
  );
  ...
  const focusMinutesToday = useMemo(() => {
    ...
    return sessions
      .filter(
        (s) =>
          s.started_at?.slice(0, 10) === todayIso && s.status === "completed",
      )
  ...
  const habitsStreakAvg = useMemo(() => {
    ...
    return Math.round(
      habits.habits.reduce((sum, h) => sum + (h.current_streak ?? 0), 0) /
    ...
    <div style={{ display: "grid", gap: "1.5rem" }}>
      <section
        style={{
          ...
          borderRadius: "20px",
          background:
            "linear-gradient(145deg, color-mix(in srgb, var(--accent) 9%, var(--surface)), var(--surface))",
          border:
      ...
      >
        ...
        >
          <div style={{ display: "grid", gap: "0.25rem", maxWidth: "58ch" }}>
            ...
            <strong style={{ fontSize: "clamp(1.05rem, 2vw, 1.35rem)" }}>
              {runway.title}
            </strong>
            <span
              style={{
        ...
        >
          ...
          >
            ...
            <strong style={{ fontSize: "1.15rem" }}>
              {plannedToday ? "Committed" : "Missing"}
            </strong>
            <span
              style={{
                fontSize: "var(--text-sm)",
            ...
            >
              {plannedToday
                ? "top tasks have been chosen today"
                : "run the ritual before you start reacting"}
            </span>
          </div>
          <div
            style={{
          ...
          >
            ...
            >
              {nextFocusTask
                ? nextFocusTask.title
                : "nothing due today is clearly leading"}
            </span>
          </div>
          <div
            style={{
          ...
          >
            ...
            <strong style={{ fontSize: "1.15rem" }}>
              {nextCalendarBlock
                ? new Date(nextCalendarBlock.start_at).toLocaleTimeString(
                    undefined,
                    {
                      hour: "2-digit",
                      minute: "2-digit",
            ...
            >
              {nextCalendarBlock
                ? nextCalendarBlock.title
                : "no event block is anchoring the day"}
            </span>
          </div>
        </div>
      </section>
          ...
          <div>
            ...
            >
              ...
              {momentum.delta_vs_yesterday !== 0 && (
                ...
                >
                  {momentum.delta_vs_yesterday > 0 ? "↑" : "↓"}{" "}
                  {Math.abs(momentum.delta_vs_yesterday)} vs yesterday
                </span>
              )}
            </div>
            <p
