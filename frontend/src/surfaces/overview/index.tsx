...
function StatCard({
  ...
  return (
    ...
    >
      ...
      {sub && (
        <span style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)' }}>{sub}</span>
      )}
    </div>
  );
}
...
export default function OverviewView(): JSX.Element {
  ...
  const habitsStreakAvg = useMemo(() => {
    ...
    return Math.round(
      habits.habits.reduce((sum, h) => sum + (h.current_streak ?? 0), 0) / habits.habits.length,
    );
  }, [habits.habits]);
      ...
      >
        ...
        >
          ...
          >
            ...
            <strong style={{ fontSize: '1.15rem' }}>
              {nextCalendarBlock
                ? new Date(nextCalendarBlock.start_at).toLocaleTimeString(undefined, {
                    hour: '2-digit',
                    minute: '2-digit',
                  })
                : 'Open'}
              ...
              {momentum.delta_vs_yesterday !== 0 && (
                <span
                  style={{
                    fontSize: 'var(--text-sm)',
                    color:
                      momentum.delta_vs_yesterday > 0 ? 'var(--state-ok)' : 'var(--state-overdue)',
                  }}
                >
                  {momentum.delta_vs_yesterday > 0 ? '↑' : '↓'}{' '}
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: 'var(--text-sm)',
                        fontWeight: 600,
                        color: positive ? 'var(--state-ok)' : 'var(--state-overdue)',
                      }}
                    >
                      {positive ? '+' : '−'}
                      {pct.toFixed(0)}%
                  ...
                  >
                    <div
                      style={{
                        height: '100%',
                        width: `${pct}%`,
                        background: positive ? 'var(--state-ok)' : 'var(--state-overdue)',
