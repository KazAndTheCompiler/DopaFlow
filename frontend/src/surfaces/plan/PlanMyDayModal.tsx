...
export default function PlanMyDayModal({
  ...
  useEffect(() => {
    ...
    try {
      ...
      setStep(
        typeof parsed.step === 'number' ? Math.max(0, Math.min(parsed.step, TOTAL_STEPS - 1)) : 0,
      );
  ...
  const yesterdayFocus = focus.sessions
    .filter(
      (s) =>
        s.status === 'completed' && (s as { ended_at?: string }).ended_at?.slice(0, 10) === yISO,
    )
    .reduce((sum, s) => sum + ((s as { duration_minutes?: number }).duration_minutes ?? 0), 0);
  const habitStreak = habits.habits.reduce((max, h) => Math.max(max, h.current_streak ?? 0), 0);
