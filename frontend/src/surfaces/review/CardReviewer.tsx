    ...
    if (sessionDone > 0) {
      return (
        ...
        >
          ...
          <strong style={{ fontSize: 'var(--text-lg)', letterSpacing: '-0.02em' }}>
            Session complete
          </strong>
          <p
            style={{
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-label={
          flipped ? 'Card back — press Space or F to flip' : 'Card front — press Space or F to flip'
        }
      ...
      >
        ...
        {card.next_review_at && !flipped && (
          <span style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)' }}>
            Next: {formatNextDueInterval(card.interval)} · Ease: {card.ease_factor.toFixed(2)} ·{' '}
            {card.reviews_done} reviews
          </span>
        )}
      </div>
      ...
      {flipped ? (
        <div style={{ display: 'grid', gap: '0.5rem' }}>
          ...
          <span
            style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', textAlign: 'center' }}
          >
