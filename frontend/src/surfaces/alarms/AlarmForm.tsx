...
const KINDS = ["tts", "youtube", "silent"] as const;
...
export function AlarmForm({ onCreate }: AlarmFormProps): JSX.Element {
  ...
  const [kind, setKind] = useState<string>("tts");
 ...
 return;
    ...
    try {
      await onCreate({
        ...
        youtube_link: kind === "youtube" && youtube_link.trim() ? youtube_link.trim() : null,
      });
      setTitle("");
      setTtsText("");
      setYoutubeLink("");
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          borderRadius: "8px",
          border: "none",
          background: title.trim() && at ? "var(--accent)" : "var(--border-subtle)",
