...
export function AlarmQueue({ alarms, activeAlarmId, onTrigger, onDelete }: AlarmQueueProps): JSX.Element {
              ...
              <div style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)", marginTop: "0.1rem" }}>
                ...
                {alarm.last_fired_at && (
                  <span style={{ marginLeft: "0.5rem", opacity: 0.7 }}>
                    · fired {new Date(alarm.last_fired_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                  </span>
                )}
              </div>
              {alarm.tts_text && (
                <div style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)", fontStyle: "italic", marginTop: "0.15rem" }}>
                  "{alarm.tts_text}"
                </div>
              )}
              {alarm.youtube_link ? <AlarmAudioPlayer youtubeUrl={alarm.youtube_link} autoPlay={active || justFiredId === alarm.id} /> : null}
            </div>
