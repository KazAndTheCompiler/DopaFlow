...
export default function TasksView({ initialView = 'list' }: TasksViewProps): JSX.Element {
  ...
  const activeCount = visibleTasks.filter(
    (task) => !task.done && task.status !== 'done' && task.status !== 'cancelled',
  ).length;
  const overdueCount = visibleTasks.filter(
    (task) =>
      !task.done && Boolean(task.due_at) && new Date(task.due_at as string).getTime() < Date.now(),
  ).length;
  ...
  const compactStats = useMemo(
    () => [
      ...
      { label: 'Done', value: doneCount, tone: 'var(--state-ok)', detail: 'already closed' },
      {
        label: 'Overdue',
        value: overdueCount,
        tone: overdueCount > 0 ? 'var(--state-overdue)' : 'var(--text-secondary)',
        detail: overdueCount > 0 ? 'needs triage' : 'under control',
      },
    ],
      ...
      <div style={{ display: 'grid', gap: '1rem' }}>
        <section
          style={{
            ...
            borderRadius: '22px',
            background:
              'linear-gradient(150deg, color-mix(in srgb, var(--accent) 8%, var(--surface)), color-mix(in srgb, var(--surface) 96%, black 4%))',
        ...
        >
          ...
          >
            <div style={{ display: 'grid', gap: '0.28rem', maxWidth: '72ch', minWidth: 0 }}>
              ...
              <strong style={{ fontSize: 'var(--text-xl)' }}>
                Move work visually instead of auditing a long list
              </strong>
              <span
                style={{
                  fontSize: 'var(--text-sm)',
              ...
              >
                Use Kanban to keep execution flowing, or switch to Eisenhower when the real problem
                is prioritization rather than volume.
              </span>
            </div>
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '0.4rem', marginLeft: 'auto', flexWrap: 'wrap' }}>
              <button
                style={viewToggleStyle(boardMode === 'kanban')}
                onClick={() => setBoardMode('kanban')}
              >
                Kanban
              </button>
              <button
                style={viewToggleStyle(boardMode === 'eisenhower')}
      ...
      >
        ...
        >
          <div style={{ display: 'grid', gap: '0.28rem', maxWidth: '72ch', minWidth: 0 }}>
            ...
            <strong style={{ fontSize: 'var(--text-xl)' }}>
              Capture quickly, then cut the list down to what should actually move
            </strong>
            <span
              style={{
                fontSize: 'var(--text-sm)',
            ...
            >
              This surface should help you offload ideas fast, filter the noise, and leave only the
              tasks that deserve attention right now.
            </span>
          </div>
          <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
            {compactStats.map((stat) => (
