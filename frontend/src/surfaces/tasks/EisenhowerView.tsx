    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: "0.6rem" }}>
        <div style={{ display: "grid", gap: "0.15rem" }}>
          ...
          <span style={{ fontSize: "var(--text-xs)", color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{cue}</span>
        </div>
        <span style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)", fontWeight: 700 }}>{tasks.length}</span>
      </div>
      <div style={{ display: "grid", gap: "0.45rem" }}>
        {tasks.length === 0 ? (
          <div style={{ fontSize: "var(--text-sm)", color: "var(--text-secondary)", padding: "0.85rem", borderRadius: "12px", border: "1px dashed var(--border-subtle)", background: "var(--surface-2)" }}>
            Nothing in this quadrant right now.
          </div>
        ) : (
          tasks.map((task) => (
            <div key={task.id} style={{ fontSize: "var(--text-sm)", padding: "0.6rem 0.7rem", borderRadius: "12px", background: "var(--surface-2)" }}>
              {task.title}
            </div>
          ))
        )}
