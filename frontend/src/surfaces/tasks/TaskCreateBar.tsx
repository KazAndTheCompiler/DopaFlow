...
export function TaskCreateBar({ onCreate }: TaskCreateBarProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    return () =>
      document.removeEventListener("focus-task-create", handleFocusEvent);
  }, []);
  ...
  return (
    ...
    >
      <div style={{ display: "grid", gap: "0.22rem" }}>
        ...
        >
          Type a plain sentence or speak it. Natural-language parsing should
          reduce cleanup work, not create another form to fill out.
        </span>
      </div>
      <div
        style={{
          ...
          >
            ...
            {preview.priority !== undefined &&
              preview.priority !== "medium" && (
                <span
                  style={chipStyle(
                    preview.priority === "urgent" || preview.priority === "high"
                      ? "var(--state-overdue)"
                      : "var(--state-warn)",
                  )}
                >
                  {preview.priority.charAt(0).toUpperCase() +
                    preview.priority.slice(1)}
                </span>
              )}
