...
export function PeerSyncBadge({
  feeds,
  onSync,
}: PeerSyncBadgeProps): JSX.Element {
  if (feeds.length === 0) {
    return <></>;
  }
  ...
  const status = isSingle
    ? feed.sync_status
    : getWorstStatus(feeds.map((f) => f.sync_status));
  const statusColor = STATUS_COLORS[status] ?? STATUS_COLORS.idle;
  const dotColor = isSingle ? feed.color : statusColor;

  const worstStatus = isSingle
    ? feed.sync_status
    : getWorstStatus(feeds.map((f) => f.sync_status));
  ...
  const staleFeeds = feeds.filter((candidate) => {
    ...
    return (
      Date.now() - new Date(candidate.last_synced_at).getTime() >
      36 * 60 * 60 * 1000
    );
  });
  const erroredFeeds = feeds.filter(
    (candidate) => candidate.sync_status === "error",
  );
