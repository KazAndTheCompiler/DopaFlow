...
export type CalendarTab = 'week' | 'day' | 'month';

export function describeFeedRepair(raw: string | null): { title: string; detail: string } {
  switch (raw) {
...
export function CalendarSharedFeedsPanel({
  ...
  sourceMeta: Array<{ id: string; label: string; color: string; readonly: boolean }>;
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'grid', gap: '0.2rem' }}>
          ...
          <span style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)' }}>
            Mirror feeds stay read-only here so future user-to-user sharing can add permissions
            cleanly instead of rewriting the model later.
          </span>
        </div>
          ...
          return (
            ...
            >
              ...
              <span
                style={{
                  fontSize: 'var(--text-sm)',
                  color: feed.last_error ? 'var(--state-overdue)' : 'var(--text-secondary)',
                  lineHeight: 1.5,
                }}
              >
                {feed.last_error
      ...
      {allSharedSourcesHidden && (
        ...
        >
          All shared calendars are hidden right now. Re-enable a source chip above to see mirrored
          events again.
        </div>
      )}
      {!allSharedSourcesHidden && sharedEvents > 0 && visibleSharedEvents === 0 && (
        ...
        >
          Shared calendars are connected, but there are no mirrored events in the visible range for
          this view.
        </div>
      )}
    </section>
