...
function topOffsetRem(event: CalendarEvent): number {
  ...
  return (
    start.getHours() * HOUR_ROW_REM +
    (start.getMinutes() / 60) * HOUR_ROW_REM +
    0.2
  );
}
...
export function DayView({
  ...
  const dayEvents = events.filter((event) =>
    sameDay(new Date(event.start_at), targetDate),
  );
  ...
  const onGridMouseMove = (e: React.MouseEvent): void => {
    ...
    const { id, type, startMouseY, startTopRem, startHeightRem, pixelsPerRem } =
      dragRef.current;
        ...
        >
          {hours.map((hour) => (
            <div
              ...
              title={
                onSlotClick
                  ? `Block ${String(hour).padStart(2, "0")}:00`
                  : undefined
              }
            />
          ))}
          ...
          {timedEvents.map((event) => {
            ...
            return (
              <div
                ...
                onMouseDown={
                  canDrag ? (e) => onMoveMouseDown(e, event) : undefined
                }
                ...
                style={{
                  ...
                  transition: isDragging
                    ? "none"
                    : "top 80ms ease, height 80ms ease",
                }}
              >
                <div
                  style={{
