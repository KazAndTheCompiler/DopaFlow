...
export function buildCalendarViewModel({
  ...
  const weekLabel = anchor.toLocaleDateString(undefined, {
    month: "long",
    year: "numeric",
  });
  const dayLabel = dayAnchor.toLocaleDateString(undefined, {
    weekday: "long",
    month: "long",
    day: "numeric",
  });
  ...
  const pendingTasks = tasks.filter(
    (task) => !task.done && task.status !== "cancelled",
  );
  const sourceMeta = [
    {
      id: "local",
  ...
  const sourceColors = Object.fromEntries(
    sourceMeta.map((source) => [source.id, source.color]),
  );
  const sourceLabels = Object.fromEntries(
    sourceMeta.map((source) => [source.id, source.label]),
  );
  ...
  const sharedEvents = events.filter((event) =>
    event.source_type?.startsWith("peer:"),
  ).length;
  const visibleSharedEvents = filteredEvents.filter((event) =>
    event.source_type?.startsWith("peer:"),
  ).length;
  const selectedEvent = selectedEventId
    ? (events.find((event) => event.id === selectedEventId) ?? null)
    : null;
  const selectedSourceKey = selectedEvent?.source_type?.startsWith("peer:")
    ? selectedEvent.source_type.slice("peer:".length)
    : "local";
  const allSharedSourcesHidden =
    peerFeeds.length > 0 &&
    peerFeeds.every((feed) => hiddenSources.includes(feed.id));
  ...
  const staleFeeds = peerFeeds.filter((feed) => {
    ...
    return (
      Date.now() - new Date(feed.last_synced_at).getTime() > staleThresholdMs
    );
  });
  const summaryChips = [
