      ...
      <div style={{ display: "grid", gap: "1rem" }}>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "0.45rem" }}>
            <span style={{ width: "9px", height: "9px", borderRadius: "999px", background: sourceColor, flexShrink: 0 }} />
          ...
          <div style={{ display: "flex", gap: "0.45rem", flexWrap: "wrap" }}>
            ...
            {event.recurrence && (
              ...
              >
                {event.recurrence.replace("FREQ=", "").toLowerCase().replace(/^\w/, (c) => c.toUpperCase())}
              </span>
            )}
            {!event.all_day && (
              <span
        ...
        <div style={{ display: "grid", gap: "0.9rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "0.75rem" }}>
            <div>
              ...
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} aria-label="Event date" style={inputStyle} disabled={readOnly} />
            </div>
            <div>
              <label style={fieldLabel}>Category</label>
              <select value={category} onChange={(e) => setCategory(e.target.value)} aria-label="Event category" style={inputStyle} disabled={readOnly}>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: "0.45rem", fontSize: "var(--text-sm)", color: "var(--text-secondary)" }}>
          ...
          {!allDay && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "0.75rem" }}>
              <div>
                ...
                <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} aria-label="Event start time" style={inputStyle} disabled={readOnly} />
              </div>
              <div>
                <label style={fieldLabel}>Ends</label>
                <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} aria-label="Event end time" style={inputStyle} disabled={readOnly} />
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <select value={recurrence} onChange={(e) => setRecurrence(e.target.value)} aria-label="Event recurrence" style={inputStyle} disabled={readOnly}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: "0.75rem", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: "0.6rem" }}>
            ...
            {!readOnly && (
              <Button onClick={() => void handleSave()} variant="primary" disabled={saving || !title.trim()}>
