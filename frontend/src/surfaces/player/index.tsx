...
export default function PlayerView(): JSX.Element {
  ...
  return (
    <div style={{ display: "grid", gap: "1rem", maxWidth: 640, margin: "0 auto" }}>
      ...
      <div style={{ ...card, gap: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <div style={{ flex: 1, overflow: "hidden" }}>
            <div style={{ fontWeight: 700, fontSize: "1rem", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {resolving ? "Resolving…" : (nowPlaying?.title ?? "Nothing playing")}
            </div>
            {resolveError && (
              <div style={{ fontSize: "var(--text-xs)", color: "var(--state-overdue)" }}>{resolveError}</div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          <button onClick={togglePlay} disabled={!nowPlaying || resolving} style={{ background: playing ? "var(--accent)" : "var(--surface-2)", color: playing ? "var(--text-inverted)" : "var(--text-primary)", border: "none", borderRadius: "50%", width: 40, height: 40, display: "grid", placeItems: "center", cursor: "pointer", fontSize: "0.72rem", fontWeight: 800, letterSpacing: "0.04em" }}>
            {playing ? "PA" : "PL"}
          </button>
          <button onClick={() => void playNext()} disabled={queue.length === 0} style={{ background: "var(--surface-2)", color: "var(--text-primary)", border: "none", borderRadius: "50%", width: 40, height: 40, display: "grid", placeItems: "center", cursor: "pointer", fontSize: "0.72rem", fontWeight: 800, letterSpacing: "0.04em" }}>
            NX
          </button>
          <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", marginLeft: "auto" }}>
            <span style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", fontWeight: 800, letterSpacing: "0.04em" }}>VOL</span>
            <input
              type="range"
              min={0}
              max={1}
      ...
      {/* Add track */}
      <div style={card}>
        <strong style={{ fontSize: "var(--text-sm)", textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)" }}>
          Add track
        </strong>
          ...
          <div style={{ display: "grid", gap: "0.35rem" }}>
            {queue.map((item, i) => (
              ...
              >
                ...
                <button
                  onClick={() => void playItem(item).then(() => removeFromQueue(i))}
                  style={{ background: "none", border: "none", cursor: "pointer", color: "var(--accent)", fontSize: "0.7rem", fontWeight: 800, letterSpacing: "0.04em" }}
                  title="Play now"
                >
                  PL
                </button>
                <button
                  ...
                  style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", fontSize: "0.7rem", fontWeight: 800, letterSpacing: "0.04em" }}
                  title="Remove"
                >
                  X
                </button>
