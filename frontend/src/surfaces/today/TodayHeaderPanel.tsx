      ...
      >
        ...
        >
          <div style={{ display: "grid", gap: "0.25rem" }}>
            ...
            <span style={{ color: "var(--text-secondary)", fontSize: "var(--text-sm)", lineHeight: 1.5 }}>{nextAction.body}</span>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "0.75rem" }}>
          <TodayStatCard label="Queue" value={focusQueue.filter((task) => !task.done).length.toString()} subtitle="ready to execute" />
          <TodayStatCard label="Backlog" value={backlogCount.toString()} subtitle="unscheduled tasks" />
          <TodayStatCard label="Done" value={completedToday.toString()} subtitle="completed on this date" />
          <TodayStatCard
            label="Next event"
            value={
              nextUpcomingEvent
                ? new Date(nextUpcomingEvent.start_at).toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })
                : "Clear"
            }
