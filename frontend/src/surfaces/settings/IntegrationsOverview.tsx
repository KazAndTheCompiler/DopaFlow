...
export default function IntegrationsOverview(): JSX.Element {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      .catch((e: unknown) =>
        setError(e instanceof Error ? e.message : 'Failed to load integration overview'),
      );
  }, []);
  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gap: '0.25rem' }}>
        ...
        >
          One place to see what is connected, what is degraded, and where to fix it without
          spelunking through five panels.
        </span>
      </div>
      {error && (
        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--state-overdue)' }}>{error}</div>
      )}
      {rows.map((row) => {
