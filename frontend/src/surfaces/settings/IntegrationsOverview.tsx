...
function inferTursoStatus(): { tone: OverviewTone; detail: string } {
  ...
  if (hasUrl) {
    return {
      tone: "Local-only",
      detail:
        "Database URL preset saved on this device. Paste a token in the Turso panel when testing.",
    };
  }
  return {
    tone: "Disabled",
    detail: "No Turso database URL preset saved locally.",
  };
}
...
function inferGitHubStatus(): { tone: OverviewTone; detail: string } {
  ...
  return {
    tone: "Disabled",
    detail: "No GitHub repo preset saved in this browser.",
  };
}
...
function toneStyle(tone: OverviewTone): CSSProperties {
  if (tone === "Connected") {
    return {
      color: "var(--state-ok)",
      background: "color-mix(in srgb, var(--state-ok) 14%, transparent)",
    };
  }
  ...
  if (tone === "Error") {
    return {
      ...
      background: "color-mix(in srgb, var(--state-overdue) 14%, transparent)",
    };
  }
  if (tone === "Local-only") {
    return {
...
export default function IntegrationsOverview(): JSX.Element {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      .then(([integrations, peerFeeds, vault]) =>
        setState({ integrations, peerFeeds, vault }),
      )
      .catch((e: unknown) =>
        setError(
          e instanceof Error
            ? e.message
            : "Failed to load integration overview",
        ),
      );
  }, []);
  ...
  const gmailTone: OverviewTone = state.integrations?.gmail_connected
    ? "Connected"
    : state.integrations
      ? "Disabled"
      : "Local-only";
  const gmailDetail = state.integrations?.gmail_connected
    ? "OAuth token stored and import path ready."
    : "No Gmail connection stored in DopaFlow.";

  const webhookTone: OverviewTone = !state.integrations?.webhooks_enabled
    ? "Disabled"
  ...
  const vaultTone: OverviewTone = state.vault?.config.vault_enabled
    ? state.vault.vault_reachable
      ? state.vault.conflicts > 0
        ? "Needs attention"
        : "Connected"
      : "Error"
    : "Disabled";
  ...
  const rows: Array<{
  ...
  }> = [
    {
      ...
      targetId: "settings-sync-sharing",
    },
    {
      name: "Gmail",
      tone: gmailTone,
  ...
  return (
    ...
    >
      <div style={{ display: "grid", gap: "0.25rem" }}>
        ...
        >
          One place to see what is connected, what is degraded, and where to fix
          it without spelunking through five panels.
        </span>
      </div>
      {error && (
        <div
          style={{ fontSize: "var(--text-xs)", color: "var(--state-overdue)" }}
        >
          {error}
        </div>
      )}
