...
export const panelStyle = {
  ...
  background:
    "linear-gradient(160deg, color-mix(in srgb, var(--surface) 88%, white 12%), var(--surface))",
  borderRadius: "20px",
...
export function parseSetupCode(
  raw: string,
): { base_url: string; token: string } | { error: string } {
  const trimmed = raw.trim();
  const separatorIndex = trimmed.indexOf("|");
  if (separatorIndex <= 0 || separatorIndex === trimmed.length - 1) {
    return {
      error: "Paste the full setup code in the form https://host/api/v2|token.",
    };
  }
  ...
  try {
    ...
    return {
      error:
        "The setup code base URL is not valid. Copy it again from the sharing screen that created it.",
    };
  }
  ...
  if (!isLikelyApiBaseUrl(baseUrl)) {
    return {
      error:
        "That URL does not look like a DopaFlow API base URL yet. It should usually end in /api/v2.",
    };
  }
  ...
  if (token.length < 12) {
    return {
      error:
        "The token looks too short. Copy the full one-time setup code again.",
    };
  }
...
export function describeConnectionError(raw: string): {
  title: string;
  detail: string;
} {
  switch (raw) {
    case "token_invalid_or_revoked":
      return {
        ...
        detail:
          "Ask the other install to create a new share token, then reconnect this feed with the fresh setup code.",
      };
    case "redirect_not_allowed":
      return {
        title: "The saved URL points somewhere unsafe",
        detail:
          "Use the direct DopaFlow API base URL from the other install. Redirects are blocked on purpose so the feed cannot silently hop hosts.",
      };
    case "invalid_feed_payload":
      return {
        title: "The remote server answered, but not with a valid calendar feed",
        detail:
          "Double-check that the base URL points to that install's DopaFlow API, usually ending in /api/v2, not to a website homepage or proxy splash page.",
      };
    case "feed_not_found":
      return {
    ...
    default:
      if (/^HTTP 401$|^HTTP 403$/i.test(raw)) {
        return {
          ...
          detail:
            "The token was likely mistyped, expired, or revoked. Reconnect with a new setup code.",
        };
      }
      if (/^HTTP 404$/i.test(raw)) {
        return {
          ...
          detail:
            "Point it to the DopaFlow API base URL, usually ending in /api/v2, then try again.",
        };
      }
      if (/^HTTP 5\d\d$/i.test(raw)) {
        return {
          ...
          detail:
            "Wait a moment and retry sync. If it keeps failing, confirm the remote app is running and reachable at the saved base URL.",
        };
      }
      if (
        ...
        return {
          ...
          detail:
            "Check the base URL, confirm the remote app is online, and make sure both installs can reach each other on the network.",
        };
      }
      if (
        raw.toLowerCase().includes("name or service not known") ||
        raw.toLowerCase().includes("nodename")
      ) {
        return {
      ...
      return {
        ...
        detail:
          "Retry the feed first. If it fails again, verify the saved base URL and replace the token with a newly generated setup code.",
      };
  }
}
