...
export function VaultTaskImportPanel({ disabled }: { disabled: boolean }): JSX.Element {
  ...
  const scan = (): void => {
    ...
    void previewTaskImport()
      ...
      .catch((error: unknown) => {
        setMsg(`Scan failed: ${error instanceof Error ? error.message : "unknown"}`);
        setScanning(false);
      });
  };
  ...
  const doImport = (): void => {
    ...
    const toImport = preview.importable.filter((_, index) => selected.has(index));
    if (!toImport.length) {
      return;
    }
    ...
    void confirmTaskImport(toImport)
      .then((response) => {
        const errorText = response.errors.length ? ` Errors: ${response.errors.slice(0, 2).join("; ")}` : "";
      ...
      .catch((error: unknown) => {
        setMsg(`Import failed: ${error instanceof Error ? error.message : "unknown"}`);
        setImporting(false);
      });
  };
                ...
                {preview.importable.map((candidate, index) => (
                  <label
                    ...
                    style={{
                      ...
                      borderRadius: "6px",
                      background: selected.has(index) ? "color-mix(in srgb, var(--accent) 8%, transparent)" : "transparent",
                      cursor: "pointer",
                  ...
                  >
                    ...
                    <div>
                      ...
                      {candidate.due_str && <span style={{ color: "var(--text-secondary)", marginLeft: "0.4rem" }}>📅 {candidate.due_str}</span>}
                      {candidate.tags.length > 0 && (
                        <span style={{ color: "var(--text-secondary)", marginLeft: "0.4rem" }}>
                          {candidate.tags.map((tag) => `#${tag}`).join(" ")}
                        </span>
                      )}
                      {candidate.project_name && <span style={{ color: "var(--text-secondary)", marginLeft: "0.4rem" }}>· {candidate.project_name}</span>}
                      <span style={{ color: "var(--text-secondary)", marginLeft: "0.4rem", fontFamily: "monospace" }}>{candidate.file_path}</span>
                    </div>
                  </label>
                ))}
              </div>
