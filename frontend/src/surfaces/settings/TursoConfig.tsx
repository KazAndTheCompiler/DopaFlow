...
export function TursoConfig(): JSX.Element {
  ...
  const handleTest = async (): Promise<void> => {
    ...
    setTestError(null);
    try {
      const result = await apiClient<{ ok: boolean; error: string | null }>("/ops/turso-test", {
        method: "POST",
    ...
    >
      <div style={{ display: "grid", gap: "0.25rem" }}>
        ...
        {configured && testState === "idle" && (
          <span style={{ fontSize: "var(--text-sm)", color: "var(--state-ok)", padding: "0.15rem 0.5rem", borderRadius: "999px", border: "1px solid currentColor" }}>
            configured
          </span>
        )}
        {testState === "ok" && (
        ...
        {testState === "error" && (
          <span style={{ fontSize: "var(--text-sm)", color: "var(--state-overdue)", padding: "0.15rem 0.5rem", borderRadius: "999px", border: "1px solid currentColor" }}>
            ER failed
          </span>
        )}
      </div>
      ...
      {testState === "error" && testError && (
        <div style={{ fontSize: "var(--text-xs)", color: "var(--state-overdue)", padding: "0.4rem 0.6rem", borderRadius: "8px", background: "color-mix(in srgb, var(--state-overdue) 10%, transparent)" }}>
          {testError}
        </div>
      )}
