...
export function CalendarPeerFeedsSection({
  ...
  onSetNewFeed: (feed: { label: string; base_url: string; token: string; color: string }) => void;
  onAddFeed: () => void;
    ...
    <section style={panelStyle}>
      ...
      >
        <div style={{ display: 'grid', gap: '0.25rem' }}>
          ...
          >
            Connect another DopaFlow calendar feed by pasting its base URL and one-time token. This
            is the integration path that later expands into richer user sharing.
          </span>
        </div>
        <Button
      ...
      {feeds.length > 0 && (
        ...
        >
          ...
          >
            Use “Sync now” when a feed looks stale. If a feed is errored, check the exact message on
            the card before removing and re-adding it.
          </span>
        </div>
      )}
      ...
      {showAddFeed && (
        ...
        >
          ...
          >
            ...
            >
              Paste the setup code first. It fills the feed URL and token automatically so you only
              need to name the connection.
            </span>
          </div>
          ...
          >
            {setupCode.trim() ? (
              'error' in parsedSetupCode ? (
                <>
                  <strong style={{ fontSize: 'var(--text-sm)', color: 'var(--state-warn)' }}>
              ...
              <>
                ...
                >
                  Use the exact one-line setup code from the other install. It should look like{' '}
                  {localApiBaseUrl}|token.
                </span>
              </>
            )}
          </div>
          ...
          >
            <label style={{ display: 'grid', gap: '0.4rem' }}>
              ...
              <input
                ...
                value={newFeed.label}
                onChange={(event) => onSetNewFeed({ ...newFeed, label: event.target.value })}
                disabled={loading}
                style={inputStyle}
              />
            </label>
            ...
            <label style={{ display: 'grid', gap: '0.4rem' }}>
              ...
              <input
                ...
                value={newFeed.token}
                onChange={(event) => onSetNewFeed({ ...newFeed, token: event.target.value })}
                disabled={loading}
                style={inputStyle}
              />
            </label>
          ...
          >
            <label style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              <input
                type="color"
                value={newFeed.color}
                onChange={(event) => onSetNewFeed({ ...newFeed, color: event.target.value })}
                disabled={loading}
              />
      ...
      {loading && feeds.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: '0.75rem' }}>
          {feeds.map((feed) => {
            const isStale =
              !feed.last_synced_at ||
              Date.now() - new Date(feed.last_synced_at).getTime() > 36 * 60 * 60 * 1000;
            const tone =
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                    <span
                      style={{
                        ...
                        boxShadow: '0 0 0 5px color-mix(in srgb, var(--surface) 72%, transparent)',
                      }}
                    />
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <Button
                        ...
                        disabled={loading}
                        variant="primary"
                        style={{ opacity: loading ? 0.6 : 1, fontSize: 'var(--text-xs)' }}
                      >
                        Save
                      </Button>
                      <Button
