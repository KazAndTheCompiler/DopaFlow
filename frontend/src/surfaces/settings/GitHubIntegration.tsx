...
export default function GitHubIntegration(): JSX.Element {
  ...
  const handleImport = async (): Promise<void> => {
    ...
    setStatus(null);
    try {
      const result = await importGitHubIssues({ token: token.trim(), repo: repo.trim(), state });
      setStatus({ type: "success", message: `Imported ${result.created} issues as tasks (${result.skipped} skipped).` });
    } catch (e) {
  ...
  return (
    <div style={card}>
      ...
      <div>
        ...
        <select
          value={state}
          onChange={(e) => setState(e.currentTarget.value as "open" | "closed" | "all")}
          style={{ width: "100%", cursor: "pointer", padding: "0.8rem 0.9rem", borderRadius: "14px", border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontFamily: "inherit", fontSize: "var(--text-sm)" }}
        >
      ...
      {status && (
        <div
          style={{
            ...
            color: status.type === "success" ? "var(--state-ok)" : "var(--state-overdue)",
          }}
        >
          {status.message}
        </div>
