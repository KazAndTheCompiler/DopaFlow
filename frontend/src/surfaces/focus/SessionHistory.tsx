...
export function SessionHistory({
  ...
  const totalMinutesToday = todayCompleted.reduce(
    ...
    >
      ...
      >
        <div style={{ display: "grid", gap: "0.18rem" }}>
          ...
          >
            Recent blocks show whether the day is actually turning into
            protected time or just planning churn.
          </span>
        </div>
      ...
      {recent.length === 0 ? (
        ...
        <div style={{ display: "grid", gap: "0.4rem" }}>
          {recent.map((session) => {
            ...
            const sameDay = session.started_at
            ...
            const statusColor =
              STATUS_COLORS[session.status] ?? "var(--text-secondary)";
            return (
              <div
                key={session.id}
                style={{
