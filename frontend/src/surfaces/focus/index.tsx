...
export default function FocusView(): JSX.Element {
  ...
  const todayCompletedSessions = sessions.filter(
    (session) =>
      ...
      session.started_at?.slice(0, 10) === todayIso,
  );
  const focusMinutesToday = todayCompletedSessions.reduce(
    (sum, session) => sum + (session.duration_minutes ?? 0),
    0,
  );
  const selectedTask = selectedTaskId
    ? tasks.tasks.find((task) => task.id === selectedTaskId)
    : null;
    ...
    <div
      style={{
        ...
        gridTemplateColumns: isCompactLayout
          ? "minmax(0, 1fr)"
          : "minmax(0, 1fr) minmax(240px, 360px)",
        alignItems: "start",
      }}
    >
      <div
      ...
      >
        ...
        <FocusPanel
          ...
          onStart={(minutes) =>
            void start(selectedTaskId || undefined, minutes)
          }
          tasks={tasks.tasks}
