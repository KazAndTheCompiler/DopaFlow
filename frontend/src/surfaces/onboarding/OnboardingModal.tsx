...
export default function OnboardingModal({
  ...
  const habitNames: Record<string, string> = Object.fromEntries(
    habits.map((h) => [h.id, h.label]),
  );
  ...
  const renderStep = (): JSX.Element => {
      ...
      return (
        <div>
          ...
          >
            ...
            >
              Your ADHD-first personal OS — tasks, habits, focus, and your brain
              in one place.
            </p>
          </div>
          <div
          ...
          >
            ...
            >
              Build a lightweight system first. You do not need to configure
              everything today.
            </span>
          </div>
          <div
          ...
          >
            {[
              { icon: "HB", label: "Habits" },
              { icon: "FC", label: "Focus" },
              { icon: "JR", label: "Journal" },
            ].map((f) => (
              <div
                key={f.label}
                style={{
                  ...
                  borderRadius: "16px",
                  background:
                    "linear-gradient(155deg, color-mix(in srgb, var(--surface) 80%, white 20%), var(--surface))",
                  border: "1px solid var(--border-subtle)",
            ...
            {habits.map((h) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: "14px",
                  border: "1.5px solid",
                  borderColor: selected.has(h.id)
                    ? "var(--accent)"
                    : "var(--border-subtle)",
                  background: selected.has(h.id)
            ...
            onKeyDown={async (e) => {
              if (e.key === "Enter") {
                ...
                try {
                  ...
                  showToast(
                    "Could not create the onboarding task. Check the server is running.",
                    "error",
                  );
                } finally {
                  setLoading(false);
    ...
    return (
      <div style={{ textAlign: "center" }}>
        ...
        >
          Open the app each morning with the Plan day ritual to set your
          intentions.
        </p>
        <Button
          onClick={async () => {
            ...
            try {
              ...
              showToast(
                "Could not create the starter habits. Check the server is running.",
                "error",
              );
            }
          }}
